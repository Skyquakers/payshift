...
export const onCCBillEvent = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
    ...
    if (eventType === CCBillEventType.NewSaleSuccess) {
        ...
        'charge.succeeded',
        {
          amount:
            currency === CurrencyCode.JPY
              ? Math.round(Number(billedInitialPrice))
              : Math.round(Number(billedInitialPrice) * 100),
      ...
      await trigger(
        'invoice.paid',
        {
          ...
          amount:
            currency === CurrencyCode.JPY
              ? Math.round(Number(billedAmount))
              : Math.round(Number(billedAmount) * 100),
          currency,
        },
        req.body
      )
